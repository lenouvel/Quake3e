...
static void SV_InitGameVM( qboolean restart ) {
	...
	// Signal the Python backend (via the bot bridge) that a game init/restart just
	// happened. The QVM (G_InitGame) resets some cvars at every init — e.g. UrT
	// 4.3.4 forces g_stamina back to 1 even in Jump mode, and the warmup->match
	// map_restart re-triggers it after bots join. SV_InitGameVM runs for BOTH a map
	// load and a map_restart, so on this signal the backend re-applies the affected
	// cvars (from conf/q3) over RCON — no engine-side config file.
	SV_BridgeBroadcastGameInit();
}
